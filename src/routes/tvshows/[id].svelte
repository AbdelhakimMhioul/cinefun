...
{#if showModalVideo}
	<OutClick on:outclick={() => (showModalVideo = false)}>
		<ModalVideo url={tvshow.videos.results[0].key} />
	</OutClick>
{/if}
...
<div class="md:flex mx-4 py-[1.875rem] text-primary-content">
	<div class="flex-shrink-0">
		<img
			...
			alt="Poster"
			class="w-full md:w-[300px] h-[300px] md:h-[450px] rounded-2xl hover:opacity-90 transition-all duration-300 ease-in-out object-fill"
		/>
	</div>
	<div class="mx-4 md:mx-[40px]">
		...
		<div class="flex">
			...
			<span class="hidden md:inline">{tvshow.episode_run_time[0]}min</span>
		</div>
		<div class="mt-5">
		...
		<div class="flex flex-col my-4 space-y-2">
			...
			<p>{tvshow.overview.substring(0, 300) + ' ...'}</p>
		</div>
		<div class="grid grid-cols-1 md:grid-cols-3 space-y-5 md:space-x-10">
			{#each tvshow.credits.cast.slice(0, 3) as actor}
...
<div class="text-primary-content">
	...
	>
		{#each tvshow.images['backdrops'].slice(0, 9) as backdrop}
			<ImageCard {backdrop} />
		{/each}
	</div>
</div>
